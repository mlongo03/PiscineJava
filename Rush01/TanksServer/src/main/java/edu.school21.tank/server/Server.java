...
public class Server {
    ...
    private void addClient(ClientHandler client) {
        clients.add(client);
        Player p = new Player(null, 0L, 0L, 0L);
        pRepo.save(p);
        Long id = p.getID();
    ...
    private static class ClientHandler implements Runnable {
        ...
        public void saveResult(String[] strs) {
            Optional<Player> p = pRepo.findById(playerId);
            if (!p.isEmpty() && strs.length != 4) {
                Player toSave = p.get();
                toSave.setShots((long) Integer.parseInt(strs[1]));
                toSave.setHits((long) Integer.parseInt(strs[2]));
                toSave.setMisses((long) Integer.parseInt(strs[3]));
                pRepo.update(toSave);
            }
        }
